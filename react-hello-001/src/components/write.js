 ...
 * 상태(state) : 값을 변화시킬 수 있거나 변화되어야 하는 변수, 객체
 * 상태가 변하면 react는 화면을 다시 그린다!! (핵심!)
 * 변수나 객체의 값이 바뀌면(변경되면)
 * 		그에 맞춰서 화면이 다시 rendering된다.
 * 만약 코드에서 변수나 객체를 수시로 바꾼다면?
 * 		화면이 무작위로 무한대로 다시 rendering 될 것이다.
...
/**
 * bbs 객체의 요소들은 input box value 속성에 지정(할당) 한다.
...
const Write = () => {
  ...
  const [bName, setB_name] = useState("이몽룡"); // useState() 리액트에서 제공하는 함수
   ...
   * 	새로 만들어진 _b_name 변수가 생성이 된다.
   * 4. react는 원래 b_name 변수와 새로 생성된 _b_name 변수를 비교하여
  ...
  const onChangeHandle = (e) => {
    ...
    // 이 두줄의 코드를 javascript의 전개연산자를 통해 한줄로 표현한 것

    // input box의 이름과 키보드로 입력한 문자열을 추출 가능
     ...
     */
    setBBsVO({ ...bbsVO, [name]: value }); // bbsVO를 전개하고 name 속성에 value값을 지정해라
  };
