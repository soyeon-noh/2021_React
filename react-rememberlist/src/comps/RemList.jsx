...
// JS 코드에서 전반적으로 사용되는 날짜, 시간 관리 객체
...
// onKeyPress 를 썼는데 안먹음. react에서 잘 안먹나봄.
function RemList() {
  /**
   * 함수를 선언하는 이유
   * 반복적으로 사용하는 코드를 하나로 묶어두고
   * 함수이름을 통해서 마치 한개의 명령문처럼 코드를 사용하는데
   * 목적이 있다.
   *
   * const f =()=>{
   * 	console.log("대한민국")
  ...
  // 상태가 없으면 최초에 rendering될 때 함수를 한번만 호출한다.
  // 변화를 감시할 상태 데이터가 없으면
  //	최초에 rendering(app 시작될때, page가 열릴때, 새로고침)될 때
  //	fetchdata를 실행한다.
  ...
  // useState안에 샘플데이터 넣으면 그거 보여주고 아니면 안보임

  // 위 코드에서 샘플데이터를 만들어 놓지않으면 오류가난다.
  // r_date와 같은 요소들이 없기때문.
  ...
  const onKeyDown = (e) => {
    ...
    if (e.keyCode === 13) {
       ...
       *
       * const arCopyAdd = [...ar, 9, 10] // 기존 배열을 복제하면서 요소를 추가
       * arCopyAdd ===> [1,2,3,4,5,9,10]
       */

      // 생성된 remember 객체 데이터를 rememberList 상태에 추가하기
      // setRememberList([rememberList.concat(remember)); // 자바스크립트 버전에 상관없이 사용가능.
      // rememberList = ?? 이 아니고
      // 기존에 rememberList 를 새로운 배열로 대치 (선언형, 불변형)
      setRememberList([...rememberList, remember]); // 전개연산자
