...
function BBsWrite({ history }) {
  ...
  const onClickSave = () => {
    ...
    const saveBBS = {
      ...bbs, //기존 bbs 복제해오고
      b_date: moment().format("YYYY[-]MM[-]DD"), //b_date 라는 칼럼과
      b_time: moment().format("HH:mm:ss"), // b_time 이라는 칼럼을 추가한다.
    };
  ...
  return (
    <div class="bbs_write">
      <div>
        <input name="b_writer" onChange={onChange} placeholder="작성자" />
      </div>
      <div>
		 ...
		 */}
        <input name="b_subject" onChange={onChange} placeholder="제목" />
      </div>
      <div>
        <input name="b_content" onChange={onChange} placeholder="내용" />
      </div>
      <div>
