...
function BookInput() {
   ...
   */
  const nextId = useRef(0); // f5를 누르지 않는 이상 초기화 되지 않는다.

  const onChangeHandler = (e) => {
    const { name, value } = e.target;
    // setBook({ ...book, [name]: value, b_id : UUID() });
    // 글자하나하나마다 함수실행되므로 비효율적
  ...
  const bookInsert = async () => {
    ...
    // 다음번 list의 id로 사용한다.
    nextId.current++; // nextId는 변하지않고 current라는 속성이 변경된다.

    /**
     * 리스트에 추가한 후 book state를 초기화하여
     * 입력창에 입력된 내용을 삭제하기
     * 이 기능을 사용하려면
     * 반드시 input box value를 사용해야 한다.
     * <input value={state}/>
     *
     * <input defaultValue={state}/>를 사용하면 효과가 없음
     */
