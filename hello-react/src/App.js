...
// ./components/hello.js 파일을 Hello 라는 컴포넌트 객체로 선언하기

// 통상
// 사용자 정의형(개발자가 임의로 만든) 컴포넌트를 사용하기 위해
// import를 할 때는 이름의 첫글자를 대문자로 사용한다.
// react를 사용하는 과정에서
// react 자체적으로 제공하는 컴포넌트와 혼동되거나 충돌하는 것을 방지하기 위함이다.
 ...
 *
 * 컴포넌트에서 포함된 tag는 반드시 open과 close가 명확해야 한다.
 * 보통  HTML에서는 <p> text 방식으로 사용하지만
 * 컴포넌트에서는 <p>text</p> 처럼 마감을 반드시 지켜야한다.
 *
 * return 문에서 사용되는 tag는 1개만 가능하다.
 * return 문에 여러개의 tag를 나열할 수 없다.
